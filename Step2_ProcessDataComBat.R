...
ERROR: Command should be: Rscript Script.R Parameter_file Working_directory
############################
The parameter file is a CSV file that contains the following:
col1 = data;
col2 = metadata;
col3 = dataset_short_name")
...
## Main function will be run on all lines of the param file
main = function(Line){

  ## Files and directories    
  dat_Filename = Line[,1]
  meta_Filename = Line[,2]
  DATASETNAME = Line[,3]
  ...
  ## Filter missing age or sex (For extra precaution)
  ...
  ## Save file
  saveName = paste0(DATASETNAME,"/data/",format(Sys.Date(),"%d%m%Y"),"_",DATASETNAME,"_ComBat_age_sex_resid_filtered.txt")
...
### Read param file and run main function
